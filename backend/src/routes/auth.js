...
router.post('/register', async (req, res) => {
  try {
    ...
    const token = createToken({ sub: user.id, email: user.email, username: user.username });
...
router.post('/login', async (req, res) => {
  try {
    ...
    const token = createToken({ sub: user.id, email: user.email, username: user.username });
    return res.json({ token, user: { id: user.id, email: user.email, username: user.username }, lastLoginAt: user.lastLoginAt });
  } catch (err) {
